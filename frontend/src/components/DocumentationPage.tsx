...
不同于传统的 ARIMA、Prophet 等统计模型，GitPulse 使用 Transformer 架构编码时序特征，同时融合项目文本描述，通过两阶段训练（预训练+微调）实现了 R²=0.7699 的预测精度，方向准确率达到 73.00%。
...
| 指标 | 数值 | 说明 |
|-----|-----|------|
| R² | 0.7699 | 决定系数，越接近1越好 |
| MSE | 0.0712 | 均方误差（标准化后） |
| DA | 73.00% | 方向准确率 |
...
| 模型 | R² | DA |
|-----|-----|-----|
| GitPulse (Transformer+Text, Fine-tuned) | 0.7699 | 73.00% |
...
> ⚠️ **注意**：Issue 数据采用**抽样分析**策略，每月最多采集 30 条 Issue，上传到 MaxKB 时最多保留 50 个月的数据。这是为了平衡分析质量和处理效率。

**实现**：

1. 从 raw_monthly_data.json 提取 Issue 列表（每月抽样 30 条）
2. 基于标签和关键词进行分类：
...
返回: {
  ...
  model_info: { R2: 0.7699, MSE: 0.0712, DA: 0.7300 }
}
\`\`\`
