    ...
    try:
        ...
        def generate():
            ...
            try:
                ...
                yield f"data: {json.dumps({'type': 'progress', 'step': 0, 'stepName': '获取 OpenDigger 基础指标', 'message': '正在获取基础指标...', 'progress': 5})}\n\n"
                opendigger = __import__('github_text_crawler', fromlist=['OpenDiggerMetrics']).OpenDiggerMetrics()
                opendigger_data, missing_metrics = opendigger.get_metrics(owner, repo)
                all_data = {'opendigger_metrics': opendigger_data}
                
                yield f"data: {json.dumps({'type': 'progress', 'step': 1, 'stepName': '获取仓库核心信息', 'message': '正在获取仓库信息...', 'progress': 15})}\n\n"
                all_data['repo_info'] = crawler.get_repo_info(owner, repo)
                
                yield f"data: {json.dumps({'type': 'progress', 'step': 2, 'stepName': '获取 README', 'message': '正在获取 README（中英文）...', 'progress': 30})}\n\n"
                all_data['readme'] = crawler.get_readme(owner, repo)
                
                yield f"data: {json.dumps({'type': 'progress', 'step': 3, 'stepName': '获取文档文件', 'message': '正在获取 docs 文档和重要 Markdown...', 'progress': 60})}\n\n"
                all_data['docs_files'] = crawler.get_docs_files(owner, repo, max_files=30, max_depth=2)
                all_data['important_md_files'] = crawler.get_important_md_files(owner, repo)
                
                yield f"data: {json.dumps({'type': 'progress', 'step': 4, 'stepName': '获取标签和贡献者', 'message': '正在获取标签和贡献者...', 'progress': 75})}\n\n"
                all_data['labels'] = crawler.get_labels(owner, repo)
                all_data['contributors'] = crawler.get_contributors(owner, repo)
                
                if missing_metrics:
                    yield f"data: {json.dumps({'type': 'progress', 'step': 5, 'stepName': '计算备用指标', 'message': '正在计算备用指标...', 'progress': 85})}\n\n"
                    fallback_metrics = crawler.calculate_fallback_metrics(owner, repo, all_data.get('repo_info'))
                    all_data['fallback_metrics'] = fallback_metrics
                
                yield f"data: {json.dumps({'type': 'progress', 'step': 6, 'stepName': '保存数据', 'message': '正在保存数据...', 'progress': 90})}\n\n"
                json_file = crawler.save_to_json(all_data, owner, repo)
                
                yield f"data: {json.dumps({'type': 'progress', 'step': 7, 'stepName': '处理数据', 'message': '正在处理数据并上传到MaxKB...', 'progress': 95})}\n\n"
                crawler.process_data(json_file, enable_maxkb_upload=True)
                
                yield f"data: {json.dumps({'type': 'progress', 'step': 8, 'stepName': '加载数据', 'message': '正在加载数据到服务...', 'progress': 98})}\n\n"
